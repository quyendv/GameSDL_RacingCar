...
void InitData() {
	...
	g_sound = Mix_LoadWAV("Sound//mission.wav");
}

void Load_Background() {
	g_background.LoadImg("Image//racingRoad_Light.png", g_screen);
...
void Load_Objects() {
	// MainObject
	Car.LoadImg("Image//Car_4.png", g_screen);
	Car.SetRect(POS_X_START_MAIN_OBJECT, POS_Y_START_MAIN_OBJECT);

	// ThreastObject
	p_threat1.LoadImg("Image//Car_1.png", g_screen);
	...
	// ExplosionObject
	exp_Car.LoadImg("Image//exp.png", g_screen);
	exp_Car.set_clip();
...
int main(int agrc, char* agrv[])
{
	...
	while (!stop)
	{
		...
		if (ret1 || ret2 || ret3) {
			...
			else if (MessageBox(NULL, L"Game Over", L"Info", MB_OK | MB_ICONSTOP) == IDOK) stop = true;
		}
